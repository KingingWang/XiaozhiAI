...
# 示例
python scripts/release.py ah-jfy-censer
```

### 编译优化

构建系统包含智能缓存机制，可显著加快编译速度：

| 场景 | 行为 | 说明 |
|------|------|------|
| 同芯片同板子 | 增量构建 | 最快，仅编译修改的文件 |
| 同芯片不同板子 | 清理 sdkconfig 后重新配置 | 复用大部分编译产物|
| 不同芯片 | 完全重新编译 | 所有组件都需要重新编译 |

### config.json 修改后必须使用 release.py

修改 `config.json` 中的 `sdkconfig_append` 配置后，必须使用 `release.py` 脚本编译才能生效。直接使用 `idf.py build` 不会读取 `config.json` 中的配置。
